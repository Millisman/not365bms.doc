...
bool bq769x0::enableCharging(uint16_t flag) {
    ...
    if(!mChargingEnabled && !data.chargingDisabled_) {
        int sys_ctrl2;
...
bool bq769x0::enableDischarging(uint16_t flag) {
    ...
    if(!mDischargingEnabled && !data.dischargingDisabled_) {
        int sys_ctrl2;
        sys_ctrl2 = readRegister(SYS_CTRL2);
...
void bq769x0::checkUser() {
    ...
    if(data.batCurrent_ > conf.Cell_OCD_mA) {
        ...
        if(mChargingEnabled && !(data.chargingDisabled_ & (1 << ERROR_USER_CHG_OCD))) {
